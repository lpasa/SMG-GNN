...
class modelImplementation_nodeClassificator(Module):
    ...
    def set_optimizer_reddit(self, lr):
        self.optimizer = torch.optim.LBFGS(self.model.parameters(), lr=lr, history_size=15)
    ...
    def train_test_model(self, input_features,labels, train_mask, test_mask, valid_mask, n_epochs, test_epoch,
        ...
        for epoch in range(n_epochs):
            ...
            model_out, logits, softmax_entropy = self.model(input_features)
            loss = self.criterion(logits[train_mask], labels[train_mask]) + softmax_entropy*10
            loss.backward()
            self.optimizer.step()
            ...
            if epoch % test_epoch == 0:
                ...
                print("training loss : ", train_loss.item(), " -- test loss : ", test_loss.item()," -- valid loss : ", val_loss.item())
                print("------")
    ...
    def train_test_model_reddit(self, input_features,labels, train_mask, test_mask, valid_mask, n_epochs, test_epoch,
        ...
        def closure():
            self.optimizer.zero_grad()
            model_out, logits, softmax_entropy = self.model(input_features)
            loss_train = self.criterion(logits[train_mask], labels[train_mask]) + softmax_entropy * 10
            loss_train.backward()
            return loss_train
    ...
    def evaluate(self, features, labels,mask):
        self.model.eval()
        with torch.no_grad():
            model_out, logits,_ = self.model(features)
